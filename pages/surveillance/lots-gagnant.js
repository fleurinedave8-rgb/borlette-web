...
export default function LotsGagnant() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setMsg('❌ ' + (e?.response?.data?.message || 'Erreur serveur'));
      setConfirm(false);
    }
    setSaving(false);
  };
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead>
              <tr style={{ background:'#f8f9fa', borderBottom:'2px solid #e5e7eb' }}>
                {['Dat','Tiraj','Lotto 3','1er Lot','2ème Lot','3ème Lot','Action'].map(h => (
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} style={{ padding:32, textAlign:'center',
                    ...
                    Aucun rezilta — klike ➕ Ajoute Rezilta
                  </td>
                </tr>
              ) : paged.map((row, i) => (
                <tr key={row._id || i}
        ...
        {showAdd && (
          <div
            ...
            <div
              ...
              {inp('loto3', 'Lotto 3 (3 chif) — optionnel', 3, '#7c3aed', 'ex: 347')}
              {inp('loto4', 'Lotto 4 (4 chif) — optionnel', 4, '#0891b2', 'ex: 4723')}

              <div style={{ display:'grid',
                gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:16 }}>
                {[
                  ['lot1','1er Lot *','#16a34a', true],
                  ['lot2','2ème Lot','#f59e0b', false],
                  ['lot3','3ème Lot','#1a73e8', false],
                ...
                <div style={{ display:'flex', gap:8, justifyContent:'center',
                  ...
                  {form.loto3 && (
                    <div style={{ textAlign:'center' }}>
                      ...
                      <div style={{ fontSize:10, color:'#888', marginTop:3 }}>Lotto 3</div>
                    </div>
                  )}
                  {[
                  ...
                  {form.loto4 && (
                    <div style={{ textAlign:'center' }}>
                      ...
                      <div style={{ fontSize:10, color:'#888', marginTop:3 }}>Lotto 4</div>
                    </div>
                  )}
                </div>
              )}
