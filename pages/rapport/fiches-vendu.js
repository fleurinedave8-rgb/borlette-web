...
export default function FichesVendu() {
  ...
  const [selFich, setSelFich] = useState(null); // modal detay
  const [wsConn,  setWsConn]  = useState(false);
  const [newFiches, setNewFiches] = useState([]); // tan reyèl
  const PER_PAGE = 15;
    ...
    <Layout>
      <div style={{ maxWidth:1100, margin:'0 auto' }}>
        ...
        <div className="card" style={{ marginBottom:14 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr auto', gap:12, marginBottom: result!==null ? 14 : 0, alignItems:'end' }}>
      ...
      {selFich && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div style={{ background:'white', borderRadius:16, padding:0, width:'100%', maxWidth:480, overflow:'hidden', boxShadow:'0 20px 60px rgba(0,0,0,0.3)' }}>
            {/* Header modal */}
            <div style={{ background:'linear-gradient(135deg,#1e293b,#0f172a)', padding:'16px 20px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
                <div style={{ color:'white', fontWeight:900, fontSize:16, fontFamily:'monospace' }}>#{selFich.ticket}</div>
                <div style={{ color:'#94a3b8', fontSize:11, marginTop:2 }}>{selFich.tirage} · {selFich.heure || fmtDate(selFich.date)}</div>
              </div>
              <button onClick={() => setSelFich(null)}
                style={{ background:'rgba(255,255,255,0.1)', border:'none', color:'white', borderRadius:8, width:32, height:32, cursor:'pointer', fontSize:16 }}>✕</button>
            </div>
            ...
            <div style={{ padding:20 }}>
              ...
              <div style={{ textAlign:'center', marginBottom:14 }}>
                {selFich.statut==='gagnant'
                  ...
                  : <span style={{ background:'#f1f5f9', color:'#475569', borderRadius:20, padding:'6px 20px', fontWeight:900 }}>💨 Pete (pa gagnant)</span>
                }
              </div>
                  ...
                  <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                    <thead>
                      <tr style={{ background:'#f8f9fa' }}>
                        {['Boule','Type','Mise'].map(h => <th key={h} style={{ padding:'8px 12px', textAlign:'left', fontWeight:700, fontSize:11, color:'#666' }}>{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {selFich.rows.map((r,i) => (
                        <tr key={i} style={{ borderBottom:'1px solid #f0f0f0' }}>
                          <td style={{ padding:'8px 12px', fontWeight:900, fontFamily:'monospace', fontSize:15, color:'#1a73e8' }}>
                            {String(r.boule||'').padStart(2,'0')}
                          </td>
                          <td style={{ padding:'8px 12px', fontSize:12, color:'#555' }}>{TYPE_LABELS[r.type]||r.type||'—'}</td>
                          <td style={{ padding:'8px 12px', fontWeight:800, color:'#16a34a' }}>{r.mise||r.montant||0} G</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
