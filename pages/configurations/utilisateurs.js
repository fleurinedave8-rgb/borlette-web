              ...
              <tbody>
                {users.map((u,i)=>(
                  <tr key={u.id} style={{ borderBottom:'1px solid #f0f0f0' }}>
                    ...
                    <td style={{ padding:'10px 14px' }}><span style={{ background:u.actif?'#d1fae5':'#fee2e2', color:u.actif?'#065f46':'#991b1b', padding:'3px 10px', borderRadius:20, fontSize:11, fontWeight:700 }}>{u.actif?'Actif':'Inactif'}</span></td>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div style={{ background:'white', borderRadius:12, padding:28, width:'100%', maxWidth:420 }}>
            ...
            <div style={{ display:'flex', gap:10, marginTop:16 }}>
              ...
              <button onClick={()=>setShowModal(false)} style={{ flex:1, padding:'11px', background:'#f0f0f0', border:'none', borderRadius:8, cursor:'pointer', fontWeight:700 }}>Annuler</button>
            </div>
          </div>
        </div>
      )}
