...
const MENU = [
  ...
  { href: '/succursal',   icon: '🏢', label: 'Succursal' },
  {
    icon: '⚙️', label: 'Configurations', sub: [
      ...
      { href:'/configurations/limites',        label:'Limites' },
...
export default function Layout({ children }) {
  ...
  const [openMenu, setOpenMenu] = typeof window !== 'undefined'
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'#f1f5f9' }}>
      ...
      {/* SIDEBAR */}
      <div style={{ width:220, background:'#1e293b', color:'white', flexShrink:0, display:'flex', flexDirection:'column', minHeight:'100vh' }}>
        {/* LOGO */}
        <div style={{ padding:'16px 14px', borderBottom:'1px solid rgba(255,255,255,0.1)', background:'#f59e0b' }}>
          <div style={{ fontWeight:900, fontSize:13, color:'#000', textAlign:'center', letterSpacing:0.5 }}>
            🎰 LA-PROBITE-BORLETTE
          </div>
          {user && (
            <div style={{ textAlign:'center', fontSize:10, color:'#333', marginTop:4 }}>
        ...
        <nav style={{ flex:1, overflowY:'auto', padding:'8px 0' }}>
          {MENU.map((item, i) => {
            if (item.sub) {
              ...
              return (
                <div key={i}>
                  <div onClick={() => setOpenMenu(isOpen ? null : i)}
                    style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 14px', cursor:'pointer', background: isActive?'rgba(245,158,11,0.2)':'transparent', color: isActive?'#f59e0b':'#cbd5e1' }}>
                    <span style={{ fontSize:12, fontWeight:700 }}>{item.icon} {item.label}</span>
                    ...
                    <div style={{ background:'rgba(0,0,0,0.2)' }}>
                      {item.sub.map((s, j) => (
                        <div key={j} onClick={() => router.push(s.href)}
                          style={{ padding:'8px 14px 8px 28px', cursor:'pointer', fontSize:11, color: router.pathname===s.href?'#f59e0b':'#94a3b8', background: router.pathname===s.href?'rgba(245,158,11,0.1)':'transparent', fontWeight: router.pathname===s.href?'700':'400' }}>
                          {s.label}
                        </div>
                      ))}
                    </div>
            ...
            return (
              <div key={i} onClick={() => router.push(item.href)}
                style={{ display:'flex', alignItems:'center', gap:8, padding:'9px 14px', cursor:'pointer', background: router.pathname===item.href?'rgba(245,158,11,0.2)':'transparent', color: router.pathname===item.href?'#f59e0b':'#cbd5e1', fontSize:12, fontWeight:700 }}>
